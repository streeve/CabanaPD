...
int main( int argc, char* argv[] )
{
...

    {
        ...
        auto init_op = KOKKOS_LAMBDA( const int, const double xp[3] )
        {
            if ( ( ( xp[0] - x_center ) * ( xp[0] - x_center ) +
                   ( xp[1] - y_center ) * ( xp[1] - y_center ) ) <
                 radius * radius )
                return false;
            return true;
        };
        ...
        double inner_outer_radius = inputs["cylindrical_hole"][3];
        double outer_radius = inputs["cylindrical_hole"][4];
        auto init_functor = KOKKOS_LAMBDA( const int pid )
        {
            ...
            rho( pid ) = rho0;
            if ( radial_pos > outer_radius * outer_radius )
            {
                type( pid ) = 3;
            }
            else if ( radial_pos > inner_outer_radius * inner_outer_radius )
            {
                type( pid ) = 1;
            }
            else
