...
// Simulate random material points for performance benchmarking.
void randomMaterial( const std::string filename )
{
    // ====================================================
    ...
    using rnd_type = Kokkos::Random_XorShift64_Pool<exec_space>;
    rnd_type pool;
    pool.init( 123456, particles.localOffset() );
    ...
    auto init_functor = KOKKOS_LAMBDA( const int pid )
    {
        ...
        if ( x( pid, 1 ) <= plane1.low[1] + horizon + 1e-10 ||
            ...
            nofail( pid ) = 1;
        // Material type.
        auto rand = pool.get_state( pid );
        type( pid ) =
            Kokkos::rand<decltype( rand ), int>::draw( rand, 0, num_types );
