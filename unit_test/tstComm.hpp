...
void testHalo()
{
    ...
    HostAoSoA aosoa_host( "host_aosoa", particles.numReference() );
    ...
    EXPECT_EQ( particles.numLocal(), init_num_particles );

    // Check all local particles unchanged.
    for ( std::size_t p = 0; p < particles.numLocal(); ++p )
    {
        for ( int d = 0; d < 3; ++d )
        {
    ...
    for ( std::size_t p = particles.numLocal(); p < particles.numReference();
          ++p )
    {
        for ( int d = 0; d < 3; ++d )
        {
    ...
    NeighListType nlist( x, 0, particles.numLocal(), delta, 1.0, mesh_min,
                         mesh_max );

    // Copy neighbors per particle to host.
    Kokkos::View<int*, TEST_MEMSPACE> num_neigh( "num_neighbors",
                                                 particles.numLocal() );
    Kokkos::RangePolicy<exec_space> policy( 0, particles.numLocal() );
    Kokkos::parallel_for(
